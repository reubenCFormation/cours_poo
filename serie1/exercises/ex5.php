...
3) Reprenez votre classe Animal crée depuis ex4 et ecrivez y une methode pour pouvoir inserer un animal (une instance de la classe Animal) en base de données (ceci n'est pas une methode statique). Hint: pensez a vos proprietés dans la classe comme etant des champs de la table animals et encore un hint: Pensez a la fonction php intval pour transformer une proprieté de type boolean en integer (vous en aurai besoin)!


4) apres avoir pu inserer plusiers animaux en base de données, trouvez un moyen de pouvoir tous les recuperer avec une methode statique! Pour faire ceci, rajoutez une proprieté privé $id a votre classe et ajoutez la au constructeur. Faites la meme chose que vous avez vu dans la classe User de notre example.
